                ...
                <div class="mt-4 px-4">
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.certificates.inputs.name')
                        </h5>
                        <span>{{ $certificate->name ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.certificates.inputs.description')
                        </h5>
                        <span>{{ $certificate->description ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.certificates.inputs.background_path')
                        </h5>
                        <x-partials.thumbnail
                            src="{{ $certificate->background_path ? \Storage::url($certificate->background_path) : '' }}"
                            size="150"
                        />
                    </div>
                </div>
